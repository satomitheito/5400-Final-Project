...
def get_sankei_art_urls(src_url):
    ...
    response.raise_for_status()
    soup = BeautifulSoup(response.content, 'html.parser')
    url = soup.find_all("article", class_="storycard border-bottom border-color-gray flex margin-bottom padding-bottom small-size col-item")
    articles = []
    for u in url:
...
def get_nhk_art_urls(src_url):
    ...
    try:
        ...
        elements = driver.find_elements(By.XPATH, '//ul[@class="content--list grid--col-single"]/li/dl/dt/a')
