...
def fetch_and_save_articles(sections, source_name, bias, num_arts):
    """
    ...
    for section, section_url in sections.items():
        logger.info(
            f"Fetching articles from {source_name} - {section} at {section_url}"
        )
        ...
        try:
            ...
            articles = newspaper_section.articles[5 : 5 + num_arts]
...
def get_am_articles():
    ...
    cnn_art = fetch_and_save_articles(cnn_sections, "CNN", "left", 30)

    logger.info("Fetching Huffington Post articles...")
    huff_art = fetch_and_save_articles(huff_sections, "Huffington Post", "neutral", 30)

    logger.info("Fetching Fox News articles...")
    fox_art = fetch_and_save_articles(fox_sections, "Fox News", "right", 30)
